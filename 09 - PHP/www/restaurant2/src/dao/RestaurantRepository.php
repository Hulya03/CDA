...
class RestaurantRepository
{
    ...
    public function createRestaurant(string $_nom, string $_adresse, float $_prix, string $_commentaire, float $_note, DateTime $_visite): bool
    {
        ...
        $prix=floatval($_prix);                     // floatval — Convertit une chaîne en nombre à virgule flottante
    ...
    public function searchAllJSON(): void                               
    {
        ...
        $stmt = $this->dbConnect->query($rq);
        $data = $stmt->fetchAll();
        $chainedata = json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);  // TRANSFORMATION DE NOTRE TABLEAU EN OBJET JSON

        file_put_contents("./API/listeResto.json", $chainedata, FILE_APPEND);
